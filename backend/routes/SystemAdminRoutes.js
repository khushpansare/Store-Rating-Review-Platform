...
// CUSTOM COMPONENT
const SystemAdminSchema = require("../models/SystemAdminSchema");
const UserSchema = require("../models/UserSchema");
const StoreOwnerSchema = require("../models/StoreOwnerSchema");

router.get("/all_store_owners", async (req, res) => {
  const all_stores_owners = await StoreOwnerSchema.find();

  res.send({
    user_details: all_stores_owners,
  });
});
...
router.get("/me/:id", async (req, res) => {
  try {
    ...
    const user = await SystemAdminSchema.findById(user_Id)
      .select("-password")
      .lean();
...
router.post("/register", async (req, res) => {
  try {
    ...
    const storeOwnerExist = await SystemAdminSchema.findOne({ email: email });
    if (storeOwnerExist)
      return res
        .status(401)
        .send(
    ...
    bcrypt.genSalt(10, (err, salt) => {
      bcrypt.hash(password, salt, async (err, hash) => {
        if (err) {
          ...
          const user = await SystemAdminSchema.create({
            name,
            email,
            password: hash,
            address,
